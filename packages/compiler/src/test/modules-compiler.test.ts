  ...
  test('compiles demo modules', () => {
    ...
    expect(modules).toEqual([
      ...
      },
      {
        ...
        path: '/src/user.ts',
        source: 'import*as runtime from"@jtdc/jtd-dialect/lib/runtime";export interface User{email:string;friends:Array<User>;name?:string;age?:number;}const validateUser:runtime.Validator=(a,b,c)=>{let d,e,f,g,h;b=b||{};c=c||"";if(runtime.checkObject(a,b,c)){runtime.checkString(a.email,b,c+"/email");d=a.friends;e=c+"/friends";if(runtime.checkArray(d,b,e)){for(f=0;f<d.length;f++){validateUser(d[f],b,e+runtime.JSON_POINTER_SEPARATOR+f);}}g=a.name;if(runtime.isNotOptional(g)){runtime.checkString(g,b,c+"/name");}h=a.age;if(runtime.isNotOptional(h)){runtime.checkInteger(h,b,c+"/age");}}return b.errors;};export{validateUser};const isUser=(value:unknown):value is User=>!validateUser(value,{shallow:true});export{isUser};',
      },
    ]);
  });
});
