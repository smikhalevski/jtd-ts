    ...
    expect(modules).toEqual([
      {
        ...
        source: 'import*as runtime from"@jtdc/jtd-dialect/lib/runtime";import{User,validateUser}from"./user";export interface Account{user:User;stats:{visitCount:number;};/**\n * The default role is guest\n */roles?:Array<Role>;}enum Role{ADMIN="admin",GUEST="guest",}export{Role};let validateAccount:runtime.Validator=(a,b,c)=>{let d,e,f,g,h;b=b||{};c=c||"";if(runtime.checkObject(a,b,c)){validateUser(a.user,b,c+"/user");d=a.stats;e=c+"/stats";if(runtime.checkObject(d,b,e)){runtime.checkInteger(d.visitCount,b,e+"/visitCount");}f=a.roles;if(runtime.isDefined(f)){g=c+"/roles";if(runtime.checkArray(f,b,g)){for(h=0;h<f.length;h++){validateRole(f[h],b,g+runtime.JSON_POINTER_SEPARATOR+h);}}}}return b.errors;};export{validateAccount};let isAccount=(value:unknown):value is Account=>!validateAccount(value,{shallow:true});export{isAccount};let validateRole:runtime.Validator=(a,b,c)=>{b=b||{};runtime.checkEnum(a,(validateRole.cache||={}).a||=["admin","guest"],b,c||"");return b.errors;};export{validateRole};let isRole=(value:unknown):value is Role=>!validateRole(value,{shallow:true});export{isRole};',
      },
      {
        definitions: expect.any(Object),
        exports: {
